"""Evaluate the new PS two-step Kovacs-style batch against the current model."""
...
BATCH_PATH = ROOT / "data" / "dsc" / "ps-kovacs-01.xlsx"
...
def extract_batch_rows() -> tuple[list[dict], list[dict]]:
    ...
    for scan_idx in all_recovery_scan_indices(segments):
        ...
        prev_scan = scan_idx
        condition = infer_condition("kovacs_eig_01", cycle_segments)
        heating_rate = segments[scan_idx].rate_c_min
        total_time = condition["t1_s"]
        if condition["mode"] == "two_step":
            total_time = float(condition["t1_s"]) + float(condition["t2_s"])
        record = {
        ...
        if condition["mode"] not in {"single_step", "two_step"}:
            record["exclusion_reason"] = "could_not_infer_condition"
        ...
        accepted_cycle_id += 1
        rows.append({
            "sample_id": f"ps_kovacs_01_{accepted_cycle_id:03d}",
            "source_file": BATCH_PATH.name,
            "cycle_id": accepted_cycle_id,
            "mode": condition["mode"],
            "T1_C": condition["T1_C"],
            "t1_s": condition["t1_s"],
            "T2_C": clean_value(condition["T2_C"]),
            "t2_s": clean_value(condition["t2_s"]),
            "total_anneal_time_s": total_time,
            "heating_rate_C_min": heating_rate,
...
def compare_to_model(rows: list[dict]) -> list[dict]:
    ...
    for i, row in enumerate(rows):
        record = {
            ...
            "T2_C": row.get("T2_C", ""),
            "t2_s": row.get("t2_s", ""),
            "total_anneal_time_s": float(row["total_anneal_time_s"]),
...
def summarize(comparison: list[dict], excluded: list[dict]) -> dict:
    ...
    for target in TARGETS:
        ...
        two_step_errors = np.array(
            [row[f"error_{target}"] for row in comparison if row["mode"] == "two_step"],
            dtype=float,
        )
        metrics[target] = {
            ...
            "two_step_mae": float(np.mean(np.abs(two_step_errors))) if len(two_step_errors) else float("nan"),
        }
    return {
        "source_file": str(BATCH_PATH.relative_to(ROOT)),
        "standard_scans_analyzed": len(comparison),
        "two_step_scans_analyzed": sum(1 for row in comparison if row["mode"] == "two_step"),
        "excluded_scans": excluded,
        "metrics": metrics,
        "notes": [
            "Metrics compare the new Kovacs/two-step batch against the model before this batch is added to training.",
            "The first scan is a 95 C, 100 s single-step correction run; the remaining accepted scans are two-step annealing runs.",
        ],
    }
...
def main() -> None:
    ...
    comparison_path = OUT_DIR / "ps_kovacs_01_prediction_vs_observed.csv"
    with comparison_path.open("w", encoding="utf-8", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=list(comparison[0]))
        writer.writeheader()
        writer.writerows(comparison)

    summary_path = OUT_DIR / "ps_kovacs_01_prediction_summary.json"
