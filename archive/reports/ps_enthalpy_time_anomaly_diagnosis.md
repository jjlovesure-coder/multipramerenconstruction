# PS enthalpy-time anomaly diagnosis after integration-window updates

> Superseded note: the current primary integration window is now `70-105 C`.
> The earlier `40-105 C` notes below are retained as historical context for
> the prior upper-bound-only correction. See
> `docs/ps_integration_window_repeatability.md` and
> `docs/ps_tnm_enthalpy_time_fit_report.md` for the current repeated-scan
> window selection and regenerated diagnostics.
